    ...
    Ownable
{
    ...
    string public name = "Verse Collection";
    ...
    function setRoyaltyWallet(address _royaltyWallet) public onlyOwner {
        require(
            _royaltyWallet != address(0),
            "setRoyaltyWallet: invalid address"
        );
    ...
    function setRoyaltyForTokenId(uint256 _tokenId, uint256 _royaltyBps)
        external
        onlyOwner
    {
     ...
     */
    function _setURI(string memory newuri) public virtual onlyOwner {
        _uri = newuri;
    }
    ...
    function mint(uint256 id, string memory tokenUri) public onlyOwner {
        _mint(owner(), id, 1, "");
        require(balanceOf(owner(), id) == 1, "mint: token not minted");
        _tokenToURI[id] = tokenUri;
        tokenCount++;
    }
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                balanceOf(to, tokenIds[i]) == 1,
                "batchMintToOneOwner: token not minted"
            );
            _tokenToURI[tokenIds[i]] = uris[i];
        }
    ...
    function safeTransferFrom2(
        ...
        require(
            balanceOf(from, id) == 0,
            "safeTransferFrom: token not transferred"
        );
